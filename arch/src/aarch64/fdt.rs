...
fn create_pci_nodes(
    ...
    for pci_device_info_elem in pci_device_info.iter() {
        ...
        // interrupt-map-map includes:
        // - A mask for PCI device number, 3 cells
        // - A mask for pin (1 ~ 4), 1 cell
        let interrupt_map_mask: Vec<u32> = vec![0xf800, 0, 0, 0x7];
        ...
        for (device_id, irq_num) in pci_irqs.iter() {
            ...
            interrupt_map.push(0);
            // Each device uses only 1 IRQ, hardcode the pin to INT#A
