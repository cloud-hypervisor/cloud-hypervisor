...
// We want to serialize the values as 10 bytes, starting with 0x regardless of the value.
// This makes it easier for humans to compare different serialized values.
    ...
    proptest! {
        ...
        fn cpuid_output_register_adjustments_serialization_works(replacements in any::<u32>(), mask in any::<u32>()) {
            ...
            let check_str_invariants = |value: &str| {
                ...
                prop_assert!(
                    !value.as_bytes()[2..].iter().any(u8::is_ascii_uppercase)
                );
                Ok(())
            };
