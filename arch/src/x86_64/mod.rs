...
/// the `kvm_hyperv` parameter is `false` then Hyper-V related MSRs are excempt from this check.
///
...
pub fn generate_required_msr_updates(
    ...
    let host_supported_msrs = hypervisor
        .get_msr_index_list()
        .map_err(|e| Error::CpuProfileMsrIndexList(e))?;

    let host_feature_msrs = hypervisor
        .get_msr_based_features()
        .map_err(|e| Error::CpuProfileFeatureMsrs(e))?;

    match cpu_profile.required_msr_updates(&host_feature_msrs, &host_supported_msrs, kvm_hyperv) {
        Ok(Some(required_updates)) => {
            // If IA32_ARCH_CAPABIILITIES is present then we need to check for compatibility.
