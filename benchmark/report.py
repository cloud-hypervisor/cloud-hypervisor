...
def main() -> int:
    ...
    groups: dict[tuple[str, str, int, int], list[tuple[float, int]]] = defaultdict(
        list
    )
    with results_path.open(newline="", encoding="utf-8") as results_file:
        for row in csv.DictReader(results_file):
            ...
            groups[key].append((float(row["elapsed_ms"]), int(row["stored_bytes"])))

    raw_by_phase: dict[str, tuple[float, float]] = {}
    for (phase, codec, _chunk_size, _workers), values in groups.items():
        if codec == "raw":
            raw_by_phase[phase] = (
    ...
    for key in sorted(groups):
        phase, codec, chunk_size, workers = key
        values = groups[key]
        ...
        raw_median_ms, raw_stored_bytes = raw_by_phase.get(phase, (math.nan, math.nan))
    ...
    print(
        f"{'phase':>9} {'codec':>14} {'chunk':>9} {'workers':>7} "
        f"{'median ms':>11} {'p95 ms':>11} {'MiB':>11} {'ratio':>8} "
        f"{'saving %':>9} {'speedup':>8}"
    )
    for row in rows:
        print(
            f"{row['phase']:>9} {row['codec']:>14} {row['chunk_size']:>9} "
            f"{row['workers']:>7} {row['median_ms']:>11} {row['p95_ms']:>11} "
            f"{row['stored_mib']:>11} {row['compression_ratio']:>8} "
