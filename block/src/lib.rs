...
#[path = "io/mod.rs"]
mod io_impl;
pub(crate) use io_impl::pwrite_all;
pub use io_impl::{aligned, async_io, fcntl, request};
pub mod formats;
mod sparse;
...
/// I/O block size, `STATX_DIOALIGN` reports the true per fd direct I/O
/// constraint accounting for the filesystem, underlying block device, and any
/// stacking such as loop or device mapper. Returns [`SECTOR_SIZE`] on any
/// failure or when the kernel does not report a value.
pub(crate) fn query_direct_alignment(fd: RawFd) -> u64 {
    ...
    if ret == 0 {
        ...
        if stx.stx_mask & STATX_DIOALIGN != 0 && stx.stx_dio_mem_align > 0 {
            return cmp::max(stx.stx_dio_mem_align, stx.stx_dio_offset_align) as u64;
        }
    }
...
impl DiskTopology {
    ...
    pub fn probe(f: &File) -> std::io::Result<Self> {
        if !is_block_device(f.as_raw_fd()) {
            ...
            if flags >= 0 && (flags & libc::O_DIRECT) != 0 {
                let alignment = query_direct_alignment(f.as_raw_fd());
