...
mod common_parallel {
    ...
    fn _test_live_migration_tcp_console(kind: ConsoleKind) {
        let disk_config = UbuntuDiskConfig::new(JAMMY_IMAGE_NAME.to_string());
        let guest = Guest::new(Box::new(disk_config));
        let kernel_path = direct_kernel_boot_path();
    ...
    #[test]
    fn test_live_migration_tcp_serial_console() {
        _test_live_migration_tcp_console(ConsoleKind::Serial);
    }

    #[test]
    fn test_live_migration_tcp_virtio_console() {
        _test_live_migration_tcp_console(ConsoleKind::Console);
    }
