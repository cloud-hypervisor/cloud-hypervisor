...
mod common_cvm {
    ...
    #[test]
    fn test_noble_simple_launch() {
        let guest = basic_cvm_guest!(NOBLE_IMAGE_NAME);
    ...
    fn test_api_http_create_boot() {
        let guest = basic_cvm_guest!(NOBLE_IMAGE_NAME).with_cpu(4);
    ...
    fn test_cpu_affinity() {
        let guest = basic_cvm_guest!(NOBLE_IMAGE_NAME).with_cpu(2);
    ...
    fn test_memory_overhead() {
        ...
        let guest =
            basic_cvm_guest!(NOBLE_IMAGE_NAME).with_memory(&format!("{guest_memory_size_kb}K"));
