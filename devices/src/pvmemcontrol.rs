...
pub enum Error {
    ...
    #[error("Guest sent us invalid command")]
    InvalidCommand(u32),
    #[error("Guest sent us invalid connection")]
        ...
        *self = match command {
            ...
            PvmemcontrolTransportCommand::Register => {
                if let PvmemcontrolState::PercpuInit(state) = state {
                    ...
                    warn!("received register without reset");
                    Self::error()
                }
            }
