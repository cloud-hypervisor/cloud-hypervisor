...
use crate::iommu::{Error as IommuError, HwIommuBackend, Invalidation, TableEntry};
...
impl Smmuv3 {
    ...
    fn handle_cfgi_ste(&mut self, sid: u32) -> Result<(), Error> {
        ...
        .map_err(|e| Error::Backend(CMD_CFGI_STE, e))
    }
    ...
    fn dispatch_command(&mut self, cmd: &Command) -> Result<(), Error> {
        if cmd.word0 & CMD_SSEC != 0
            && matches!(
                cmd.opcode,
                CMD_CFGI_STE | CMD_CFGI_STE_RANGE | CMD_CFGI_CD | CMD_CFGI_CD_ALL
            )
        {
...
mod tests {
    ...
    impl HwIommuBackend for FailingBackend {
        fn install_table_entry(
            &self,
            _device_id: u32,
            _entry: TableEntry,
