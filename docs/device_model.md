...
Generic vhost-user devices forward all configuration space accesses to the
backend. The backend must negotiate the `VHOST_USER_PROTOCOL_F_CONFIG` feature
to indicate it can handle these accesses. This also means that the backend
can emulate them as it desires. For instance, the tag of a virtio-fs device
can be set on the virtiofsd command line, whereas the built-in virtio-fs support
