...
daemon can transform the payload on the fly (encrypt, compress, stream to
object storage, etc.) without ever touching local disk.
...
- On snapshot, CH acts as the migration *sender* and the daemon acts as the
  *receiver*. The source VM shuts down on success, exactly as it would for a
  local live migration. Memory is transferred zero-copy via SCM_RIGHTS:
  CH hands the daemon one fd per guest-memory slot.
...
Operationally, this means offload is driven through the existing
...
endpoints. Requires the VM to be configured with shared-memory backing,
which is the same precondition that applies to local live migration today.
...
#    `offload_daemon` and persists snapshot data to a local directory; a
#    real daemon would encrypt/upload instead.
...
```bash
# 1. Start a fresh Cloud Hypervisor process (no VM yet).
./cloud-hypervisor --api-socket /tmp/cloud-hypervisor.sock

# 2. Tell CH to listen for an inbound migration from the offload daemon.
./ch-remote --api-socket /tmp/cloud-hypervisor.sock \
    receive-migration unix:/tmp/restore.sock &
...
purpose is to back the offload integration test and to serve as a worked
example for daemon authors. Use it as a template, not a production
backend.
...
- TDX guests inherit the live-migration restriction: offload is not
  supported under TDX.
- **Reference-daemon-only**: virtio-mem zones are not supported. The
  reference `offload_daemon` doesn't carry the plugged-blocks state or
  do sparse (plugged-only) byte transfer, so it would silently lose
  plugged-in pages on restore. It refuses such snapshots with an
  explicit error rather than corrupting them. A production daemon can
  lift this restriction by porting the in-tree virtio-mem handling
  (see `MemoryManager::memory_range_table` and
  `VirtioMemZone::plugged_ranges`).
