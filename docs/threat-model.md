...
to crash due to bugs in the PCI emulation code.  These bugs should still
be fixed, but they are not considered vulnerabilities at this time.
...
The environment Cloud Hypervisor is running in must follow best practices
for isolating untrusted code. This includes, but is not limited to,
enabling mitigations for speculative execution vulnerabilities
(including disabling SMT if necessary).
