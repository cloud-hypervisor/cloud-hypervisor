...
impl hypervisor::Hypervisor for KvmHypervisor {
    ...
    fn create_vm(&self, _config: HypervisorVmConfig) -> hypervisor::Result<Arc<dyn vm::Vm>> {
        ...
        #[cfg(target_arch = "aarch64")]
        let nested = if _config.nested {
            if self.kvm.check_extension_raw(KVM_CAP_ARM_EL2.into()) <= 0 {
                return Err(hypervisor::HypervisorError::NestedVirtualizationUnsupported);
            }
            true
        } else {
            false
