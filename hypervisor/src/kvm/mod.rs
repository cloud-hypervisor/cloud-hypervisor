...
impl cpu::Vcpu for KvmVcpu {
    ...
    fn set_sev_control_register(
        ...
        sregs.cs = SegmentRegister::from(vmsa.cs).into();
        sregs.ds = SegmentRegister::from(vmsa.ds).into();
        sregs.es = SegmentRegister::from(vmsa.es).into();
        sregs.fs = SegmentRegister::from(vmsa.fs).into();
        sregs.gs = SegmentRegister::from(vmsa.gs).into();
        sregs.ss = SegmentRegister::from(vmsa.ss).into();
        sregs.tr = SegmentRegister::from(vmsa.tr).into();
        sregs.ldt = SegmentRegister::from(vmsa.ldtr).into();
