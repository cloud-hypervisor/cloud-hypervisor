...
// KVM_X86_SNP_VM is defined in Linux v6.11+ (arch/x86/include/uapi/asm/kvm.h)
// but is not yet available in the kvm-bindings crate.
pub const KVM_X86_SNP_VM: u32 = 4;
#[cfg(feature = "sev_snp")]
use igvm_defs::PAGE_SIZE_4K;
#[cfg(feature = "sev_snp")]
use kvm_bindings::{KVM_MEMORY_ATTRIBUTE_PRIVATE, kvm_memory_attributes, kvm_segment as Segment};
...
impl vm::Vm for KvmVm {
    ...
    unsafe fn create_user_memory_region(
        ...
        // guest_memfd is only used on x86_64 (SEV-SNP, TDX) for now
        #[cfg(target_arch = "x86_64")]
                ...
                memfd = if kvm_guest_memfd_supported {
                    ...
                    unsafe {
                        Some(OwnedFd::from_raw_fd(
                            fd.create_guest_memfd(kvm_create_guest_memfd {
                                size: VIRTUAL_ADDRESS_SIZE_IN_BITS as u64,
                                ..Default::default()
                            })
...
impl cpu::Vcpu for KvmVcpu {
                ...
                VcpuExit::Hypercall(hypercall) => {
                    ...
                    match hypercall.nr {
                        KVM_HC_MAP_GPA_RANGE => {
                            info!("Handling KVM_HC_MAP_GPA_RANGE hypercall");
    ...
    fn set_sev_control_register(&self, _vmsa_pfn: u64) -> cpu::Result<()> {
        // KVM does not use vmsa_pfn directly; vCPU state is set via
        // setup_sev_snp_regs instead.
        Ok(())
    }
