...
impl vm::Vm for KvmVm {
    ...
    fn create_vcpu(
        ...
        //
        // Calling `ARCH_GET_XCOMP_GUEST_PERM` at runtime does not affect existing VCPUs,
        // since their `fpstate` has already been allocated at creation time.
        // It only affects VCPUs created afterwards.
...
impl KvmVcpu {
    ...
    pub fn get_xsave2(&self) -> cpu::Result<XsaveState> {
        // SAFETY: The caller guarantees that xsave_size > 0
        assert!(self.xsave_size > 0);
