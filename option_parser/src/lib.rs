...
impl OptionParser {
    ...
    /// Parses the `addr` option of PCI devices and returns the PCI device as
    /// well as the function ID
    ///
    /// Returns a tuple consisting of the parsed IDs for device and function in
    /// this order. Returns an error if the supplied `addr` values cannot be
    /// parsed to [`u8`]. The tuple might consist of two times [`None`] if
    /// `addr` was not provided.
    pub fn get_pci_device_function(
        &self,
    ) -> OptionParserResult<Option<(u8 /* Device ID */, u8 /* Function ID */)>> {
        if let Some(addr_str) = self.get("addr") {
            ...
            // We also accept a hex number with `0x` prefix, but need to strip
            // it before conversion in case it's present.
            ...
            // There are only 32 device IDs per PCI bus, and the first ID (0) is
            // always used for the root device. A device ID must therefore be in
            // the inclusive range of [1, 31].
            if (device_id == 0) || (device_id > 31) {
                return Err(OptionParserError::InvalidValue(format!(
                    "expected a device ID in the inclusive range of 1 to 31, found {device_id}"
                )));
            }
            ...
            // Currently, CHV only supports single-function devices. These are
            // mapped to function ID 0 in all cases, so we disallow the
            // assignment of any other function ID.
            if function_id != 0 {
                return Err(OptionParserError::InvalidValue(format!(
                    "multi-function devices currently not supported; expected 0 got {function_id}"
                )));
            }
            Ok(Some((device_id, function_id)))
        } else {
            Ok(None)
        }
    }
...
mod unit_tests {
    ...
    fn test_option_parser() {
        ...
        // Valid with `0x` hex prefix
        parser.parse("addr=0x0A.0").unwrap();
        assert_eq!(
            Some((0xa_u8, 0)),
            parser.get_pci_device_function().expect("should be valid")
        );
        parser.parse("addr=0x0b.0").unwrap();
        assert_eq!(
            Some((0xb_u8, 0)),
            parser.get_pci_device_function().expect("should be valid")
        );
