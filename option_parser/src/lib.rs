...
impl OptionParser {
    ...
    pub fn get_pci_device_function(
        ...
        if let Some(addr_str) = self.get("addr") {
            ...
            // There are only 32 device ID per PCI bus and the first ID (0) is always used for the root device. A device
            // id must therefore be in the inclusive range of [1, 31].
