...
impl VfioCommon {
    ...
    pub(crate) fn enable_msix(&self) -> Result<(), VfioPciError> {
        if let Some(msix) = &self.interrupt.msix {
            ...
            debug!("enable msix msix_table entries {} unmasked {}",
                msix.bar.table_entries.len(), vector_num);
            ...
            // We intended to set -1 as an invalid fd, however, to avoid
            // assert in OwnedFd, set -2 here.
            if 0 == vector_num {
    ...
    pub(crate) fn write_bar(
        ...
        if let Some(region) = self.find_region(addr) {
            ...
            if self.interrupt.msix_vector_num() > old_vectors {
                debug!("resizing vectors {} to {}", old_vectors, self.interrupt.msix_vector_num());
                self.disable_msix();
                self.enable_msix().ok()?;
            }
        }
