...
errors to the guest using `VIRTIO_BLK_S_IOERR`. It improves user
...
* A default IP (`192.168.249.1`) and mask (`255.255.255.0`) now are
 assigned  to the `virtio-net` device if no value is specified by
 users. Such behavior is now deprecated.  Users of this behavior will
