...
if [ ! -f "$OS_RAW_IMAGE" ]; then
    pushd $WORKLOADS_DIR
    time qemu-img convert -p -f qcow2 -O raw $OS_IMAGE_NAME $OS_RAW_IMAGE_NAME
    popd
fi
...
if [ ! -f "$VMLINUX_IMAGE" ]; then
    SRCDIR=$PWD
    pushd $WORKLOADS_DIR
    time wget --quiet https://cdn.kernel.org/pub/linux/kernel/v5.x/linux-5.0.21.tar.xz
    time tar xf linux-5.0.21.tar.xz
    pushd linux-5.0.21
...
if [ ! -f "$VMLINUX_CUSTOM_IMAGE" ]; then
    SRCDIR=$PWD
    pushd $WORKLOADS_DIR
    time git clone --depth 1 "https://github.com/sboeuf/linux.git" -b "virtio-pmem_and_virtio-fs" $LINUX_CUSTOM_DIR
...
if [ ! -f "$VIRTIOFSD" ]; then
    pushd $WORKLOADS_DIR
    time wget --quiet $VIRTIOFSD_URL -O "virtiofsd"
    chmod +x "virtiofsd"
...
time cargo test --features "integration_tests" -- --test-threads=1
