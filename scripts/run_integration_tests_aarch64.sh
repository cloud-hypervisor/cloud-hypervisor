...
update_workloads() {
    ...
    NOBLE_OS_QCOW2_IMAGE_UNCOMPRESSED_NAME="noble-server-cloudimg-arm64-custom-20260601-0.qcow2"
    NOBLE_OS_QCOW2_UNCOMPRESSED_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW2_IMAGE_UNCOMPRESSED_NAME"

    for required in "$NOBLE_OS_RAW_IMAGE" "$NOBLE_OS_QCOW2_UNCOMPRESSED_IMAGE" \
    ...
    NOBLE_OS_QCOW2_ZLIB_FILE_IMAGE_NAME="noble-server-cloudimg-arm64-custom-20260601-0-zlib.qcow2"
    NOBLE_OS_QCOW2_ZLIB_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW2_ZLIB_FILE_IMAGE_NAME"
    if [ ! -f "$NOBLE_OS_QCOW2_ZLIB_FILE_IMAGE" ]; then
        pushd "$WORKLOADS_DIR" || exit
        time qemu-img convert -c -f raw -O qcow2 -o compression_type=zlib \
            "$NOBLE_OS_RAW_IMAGE" $NOBLE_OS_QCOW2_ZLIB_FILE_IMAGE_NAME
        popd || exit
    fi

    NOBLE_OS_QCOW2_ZSTD_FILE_IMAGE_NAME="noble-server-cloudimg-arm64-custom-20260601-0-zstd.qcow2"
    NOBLE_OS_QCOW2_ZSTD_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW2_ZSTD_FILE_IMAGE_NAME"
    if [ ! -f "$NOBLE_OS_QCOW2_ZSTD_FILE_IMAGE" ]; then
        pushd "$WORKLOADS_DIR" || exit
        time qemu-img convert -c -f raw -O qcow2 -o compression_type=zstd \
            "$NOBLE_OS_RAW_IMAGE" $NOBLE_OS_QCOW2_ZSTD_FILE_IMAGE_NAME
        popd || exit
    fi

    NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE_NAME="noble-server-cloudimg-arm64-custom-20260601-0-backing-zstd.qcow2"
    NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE_NAME"
    if [ ! -f "$NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE" ]; then
        pushd "$WORKLOADS_DIR" || exit
        time qemu-img create -f qcow2 -b "$NOBLE_OS_QCOW2_ZSTD_FILE_IMAGE" -F qcow2 $NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE_NAME
        popd || exit
    fi

    NOBLE_OS_QCOW_BACKING_UNCOMPRESSED_FILE_IMAGE_NAME="noble-server-cloudimg-arm64-custom-20260601-0-backing-uncompressed.qcow2"
    NOBLE_OS_QCOW_BACKING_UNCOMPRESSED_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW_BACKING_UNCOMPRESSED_FILE_IMAGE_NAME"
    if [ ! -f "$NOBLE_OS_QCOW_BACKING_UNCOMPRESSED_FILE_IMAGE" ]; then
        pushd "$WORKLOADS_DIR" || exit
        time qemu-img create -f qcow2 \
            -b "$NOBLE_OS_QCOW2_UNCOMPRESSED_IMAGE" \
            -F qcow2 $NOBLE_OS_QCOW_BACKING_UNCOMPRESSED_FILE_IMAGE_NAME
        popd || exit
    fi

    NOBLE_OS_QCOW_BACKING_RAW_FILE_IMAGE_NAME="noble-server-cloudimg-arm64-custom-20260601-0-backing-raw.qcow2"
    NOBLE_OS_QCOW_BACKING_RAW_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW_BACKING_RAW_FILE_IMAGE_NAME"
    if [ ! -f "$NOBLE_OS_QCOW_BACKING_RAW_FILE_IMAGE" ]; then
        pushd "$WORKLOADS_DIR" || exit
        time qemu-img create -f qcow2 \
            -b "$NOBLE_OS_RAW_IMAGE" \
            -F raw $NOBLE_OS_QCOW_BACKING_RAW_FILE_IMAGE_NAME
        popd || exit
    fi
