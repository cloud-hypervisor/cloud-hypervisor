...
for required in "$FW" "$WORKLOADS_DIR/CLOUDHV.fd" "$NOBLE_OS_IMAGE" \
...
NOBLE_OS_QCOW_ZLIB_FILE_IMAGE_NAME="noble-server-cloudimg-amd64-custom-20260601-0-zlib.qcow2"
NOBLE_OS_QCOW_ZLIB_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW_ZLIB_FILE_IMAGE_NAME"
if [ ! -f "$NOBLE_OS_QCOW_ZLIB_FILE_IMAGE" ]; then
    pushd "$WORKLOADS_DIR" || exit
    time qemu-img convert -c -f raw -O qcow2 -o compression_type=zlib \
        "$NOBLE_OS_RAW_IMAGE" $NOBLE_OS_QCOW_ZLIB_FILE_IMAGE_NAME
    popd || exit
fi

NOBLE_OS_QCOW_ZSTD_FILE_IMAGE_NAME="noble-server-cloudimg-amd64-custom-20260601-0-zstd.qcow2"
NOBLE_OS_QCOW_ZSTD_FILE_IMAGE="$WORKLOADS_DIR/$NOBLE_OS_QCOW_ZSTD_FILE_IMAGE_NAME"
if [ ! -f "$NOBLE_OS_QCOW_ZSTD_FILE_IMAGE" ]; then
    pushd "$WORKLOADS_DIR" || exit
    time qemu-img convert -c -f raw -O qcow2 -o compression_type=zstd \
        "$NOBLE_OS_RAW_IMAGE" $NOBLE_OS_QCOW_ZSTD_FILE_IMAGE_NAME
    popd || exit
fi

NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE_NAME="noble-server-cloudimg-amd64-custom-20260601-0-backing-zstd.qcow2"
...
if [ ! -f "$NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE" ]; then
    ...
    time qemu-img create -f qcow2 \
        -b "$NOBLE_OS_QCOW_ZSTD_FILE_IMAGE" \
        -F qcow2 $NOBLE_OS_QCOW_BACKING_ZSTD_FILE_IMAGE_NAME
    popd || exit
fi

NOBLE_OS_QCOW_BACKING_UNCOMPRESSED_FILE_IMAGE_NAME="noble-server-cloudimg-amd64-custom-20260601-0-backing-uncompressed.qcow2"
...
NOBLE_OS_QCOW_BACKING_RAW_FILE_IMAGE_NAME="noble-server-cloudimg-amd64-custom-20260601-0-backing-raw.qcow2"
...
mkdir -p "$VFIO_DIR"
cp "$NOBLE_OS_RAW_IMAGE" "$VFIO_DIR"
cp "$FW" "$VFIO_DIR"
