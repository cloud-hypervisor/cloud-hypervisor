...
"#
    ...
    let host_message = c_str.to_string_lossy().to_string();
    // Check the probe message in host and guest
    assert_eq!(host_message, guest_message);
...
mod ivshmem {
    ...
    fn _test_live_migration_ivshmem(local: bool) {
        ...
        let r = std::panic::catch_unwind(|| {
            ...
            Command::new("echo")
                .args([format!("'ivshmem device init data' > {ivshmem_file_path}").as_str()])
                .status()
                .unwrap();
        ...
        let r = std::panic::catch_unwind(|| {
            ...
            Command::new("echo")
                .args([
                    format!("'ivshmem device data after migration' > {ivshmem_file_path}").as_str(),
                ])
                .status()
                .unwrap();
