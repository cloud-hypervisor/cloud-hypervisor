...
pub enum BalloonStatsError {
    ...
    #[error("Balloon statistics queue is not ready")]
    QueueNotReady,
...
struct StatsRequestPermit {
    pending: Option<Arc<AtomicBool>>,
}

impl StatsRequestPermit {
...
impl Drop for StatsRequestPermit {
    fn drop(&mut self) {
        self.release();
    }
}
...
impl BalloonEpollHandler {
    ...
    fn process_stats_queue(&mut self) -> result::Result<(), Error> {
        ...
        match &mut stats_handler.state {
            BalloonStatsState::WaitingForInitialDescriptor => {
                stats_handler.state = BalloonStatsState::Idle { descriptor_index };
            }
            BalloonStatsState::Pending { request } => {
                ...
                let _ = request.response_sender.try_send(response);
                stats_handler.state = BalloonStatsState::Idle { descriptor_index };
            }
    ...
    fn process_stats_request(&mut self, mut request: StatsRequest) -> result::Result<(), Error> {
        let (queue_index, descriptor_index) = {
            ...
            match &mut stats_handler.state {
                BalloonStatsState::WaitingForInitialDescriptor => {
                    ...
                    let _ = request
                        .response_sender
                        .try_send(Err(BalloonStatsError::QueueNotReady));
                    return Ok(());
                }
                ...
                BalloonStatsState::Pending { .. } => {
                    error!("Unexpected balloon statistics request while another is pending");
                    return Ok(());
                }
            }
        };
...
impl Balloon {
    ...
    pub fn begin_stats_request(
        ...
        let permit = StatsRequestPermit::acquire(Arc::clone(&self.stats_request_pending))
...
mod tests {
    ...
    fn create_stats_request() -> (StatsRequest, Receiver<BalloonStatsResult>, Arc<AtomicBool>) {
        let pending = Arc::new(AtomicBool::new(false));
        let permit = StatsRequestPermit::acquire(Arc::clone(&pending)).unwrap();
    ...
    fn stats_request_rejects_pending_request() {
        ...
        // Caller timeout does not release the permit while the worker still
        // owns the request.
        assert!(balloon.stats_request_pending.load(Ordering::Acquire));
    ...
    fn stats_request_returns_fresh_descriptor() {
        ...
        assert_eq!(
            response_receiver.try_recv().unwrap().unwrap().free_memory,
            Some(200)
        );
        assert!(matches!(
            &handler.stats_handler.as_ref().unwrap().state,
    ...
    #[test]
    fn stats_request_fails_before_initial_descriptor() {
        const QUEUE_ADDRESS: GuestAddress = GuestAddress(0x1_0000);
        const STATS_ADDRESS: GuestAddress = GuestAddress(0x1_000);
        ...
        assert!(matches!(
            ...
            Err(BalloonStatsError::QueueNotReady)
        ));
        assert!(matches!(
            &handler.stats_handler.as_ref().unwrap().state,
            BalloonStatsState::WaitingForInitialDescriptor
    ...
    #[test]
    fn stats_request_permit_survives_caller_timeout() {
        const QUEUE_ADDRESS: GuestAddress = GuestAddress(0x1_0000);
        const STATS_ADDRESS: GuestAddress = GuestAddress(0x1_000);
        ...
        StatsRequestPermit::acquire(pending).unwrap();
    }
    ...
    fn stats_request_propagates_queue_error() {
        ...
        let (request, _, pending) = create_stats_request();

        assert!(matches!(
            handler.process_stats_request(request),
