...
impl ConsoleEpollHandler {
    ...
    fn tcp_connect(&mut self, helper: &mut EpollHelper) {
        let Endpoint::TcpConnect { addr, .. } = &self.endpoint else {
            return;
        };
        let addr = *addr;
        ...
        let Some(fd) = console.client_fd() else {
            return;
        };
        if let Err(e) = helper.add_event_custom(fd, FILE_EVENT, epoll::Events::EPOLLIN) {
...
impl EpollHelperHandler for ConsoleEpollHandler {
    ...
    fn handle_timeout(&mut self, helper: &mut EpollHelper) -> Result<(), EpollHelperError> {
        if matches!(&self.endpoint, Endpoint::TcpConnect { .. }) {
            let connected = self
                .socket_console
                .as_ref()
