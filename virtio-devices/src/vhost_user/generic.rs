...
impl Generic {
    /// Create a new virtio-generic device.
...
impl VirtioDevice for Generic {
    ...
    fn read_config(&self, offset: u64, mut data: &mut [u8]) {
        if let Some(vu) = &self.vu_common.vu {
            ...
            {
                error!("Failed getting vhost-user-generic configuration: {}", e)
            }
        }
    }
...
impl Snapshottable for Generic {
    fn id(&self) -> String {
        self.id.clone()
    }
...
impl Transportable for Generic {}

impl Migratable for Generic {
