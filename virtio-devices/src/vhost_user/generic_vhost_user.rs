...
impl GenericVhostUser {
    ...
    pub fn new(
        ...
        // One cannot have a virtio device with no queues.
        if num_queues == 0 {
            error!("virtio device must have at least one queue");
            return Err(Error::BadQueueNum);
        }

        // A virtio-FS device with only one queue is invalid and causes
        // some versions of Linux to crash.  Return a more useful error
        // message instead.  26 is the ID for virtio-fs.
        if device_type == 26 && num_queues == 1 {
            error!("vhost-user-fs device must have at least 2 queues, but got only 1");
            return Err(Error::BadQueueNum);
        }
...
since the backend only supports {backend_num_queues}\n",
        ...
        // Configuration space access is entirely the responsibility of the backend.
        if (VhostUserProtocolFeatures::CONFIG.bits() & acked_protocol_features) == 0 {
            error!(
                "generic vhost-user backend does not support config space access \
(negotiated protocol features 0x{acked_protocol_features:x})"
            );
            return Err(Error::VhostUserConfigurationSpaceAccessNotSupported);
        }
