...
impl VirtioDevice for GenericVhostUser {
    ...
    fn read_config(&self, offset: u64, data: &mut [u8]) {
        assert!(data.len() <= u32::MAX as usize, "too much data to read");
        assert!(offset <= u32::MAX as u64, "too large offset");
        assert!(
            data.len() as u64 + offset <= u32::MAX as u64,
            "too large data + offset"
        );
        if let Err(e) = self
            .vu_common
            .vu
