...
impl PartiallyReadCommand {
    pub const BUF_SIZE: usize = PARTIALLY_READ_COMMAND_BUF_SIZE;
}
...
impl VsockMuxer {
    ...
    fn parse_port_from_read_command(command: &PartiallyReadCommand) -> Result<u32> {
        ...
        if !command.buf.starts_with("connect ".as_bytes())
            && !command.buf.starts_with("CONNECT ".as_bytes())
        {
        ...
        let port_string_as_bytes = &command.buf["connect ".len()..new_line_position];
        let port_string =
            std::str::from_utf8(port_string_as_bytes).map_err(|_| Error::InvalidPortRequest)?;
    ...
    fn read_local_stream_port(
        ...
        // read only a single byte at a time, this is really inefficient but prevents us
        ...
        let read_bytes = stream
            .read(
                &mut command.buf[command.len..min(command.len + 1, PartiallyReadCommand::BUF_SIZE)],
            )
...
mod tests {
    ...
    impl PartiallyReadCommand {
        ...
        fn from_str(s: &str) -> Self {
            ...
            for (i, elem) in input_bytes
                .iter()
                .enumerate()
                .take(min(input_bytes.len(), Self::BUF_SIZE))
            {
                command.buf[i] = *elem;
                command.len = i + 1;
            }
    ...
    fn test_parse_command() {
        ...
        let max_buf_length = "CONNECT                        1";
        assert_eq!(max_buf_length.len(), PartiallyReadCommand::BUF_SIZE);
        assert!(matches!(
            VsockMuxer::parse_port_from_read_command(&PartiallyReadCommand::from_str(
                max_buf_length
            )),
