...
fn arm_boot_arch(nested: bool) -> u16 {
    // PSCI_COMPLIANT | PSCI_USE_HVC for EL1 guests; EL2 guests must use SMC.
    if nested { 1 } else { 3 }
}
...
fn create_facp_table(
    ...
    #[cfg(target_arch = "aarch64")]
    // ARM_BOOT_ARCH: enable PSCI and use HVC only for ordinary EL1 guests.
    facp.write(129, arm_boot_arch(_nested));
...
fn create_acpi_tables_internal(
    ...
    let facp = create_facp_table(dsdt_addr, device_manager, legacy_acpi_pm1a, nested);
...
mod tests {
    ...
    #[test]
    fn test_arm_boot_arch() {
        assert_eq!(arm_boot_arch(false), 3);
        assert_eq!(arm_boot_arch(true), 1);
    }
