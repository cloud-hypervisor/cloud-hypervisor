...
pub enum Error {
    ...
    /// VhostUser min queues is missing
    #[error("Error parsing --user-vhost-device: default queues missing")]
...
impl UserVhostDeviceConfig {
    ...
    \"virtio_id=<id>,socket=<socket_path>,\
    queue_sizes=<a list of queue sizes>,id=<device_id>,pci_segment=<segment_id>,\
    min_queues=<minimum_number_of_queues>,avail_features=<available features>,\
    default_queues=<default number of queues if MQ is not supported>\"";
    ...
    pub fn parse(vhost_user: &str) -> Result<Self> {
        ...
        let default_queues = parser
            .convert("default_queues")
            .map_err(Error::ParseUserVhost)?
            .ok_or(Error::ParseUserVhostDefaultQueuesMissing)?;
        let id = parser.get("id");
        let pci_segment = parser
            .convert("pci_segment")
            .map_err(Error::ParseUserVhost)?
...
avail_features={avail_features},default_queues={default_queues}"
        ));
        if pci_segment <= u16::MAX as _
            && virtio_id <= u32::MAX as _
            && min_queues <= u16::MAX as _
            && default_queues <= u16::MAX as _
            && queue_sizes.0.iter().all(|&f| f <= u16::MAX.into())
        {
            assert_eq!(
                ...
                UserVhostDeviceConfig {
                    ...
                    default_queues: u16::try_from(default_queues).unwrap(),
