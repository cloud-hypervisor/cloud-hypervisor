    ...
    fn move_bar(
        ...
        if let Some(virtio_pci_dev) = any_dev.downcast_ref::<VirtioPciDevice>() {
            ...
            if bar_addr == new_base {
                virtio_pci_dev.ioeventfds(old_base, &mut |event, addr| {
                    let io_addr = IoEventAddress::Mmio(addr);
                    self.vm.unregister_ioevent(event, &io_addr).map_err(|e| {
                        io::Error::other(format!("failed to unregister ioevent: {e:?}"))
                    })
                })?;
                virtio_pci_dev.ioeventfds(new_base, &mut |event, addr| {
                    let io_addr = IoEventAddress::Mmio(addr);
                    self.vm
                        .register_ioevent(event, &io_addr, None)
                        .map_err(|e| io::Error::other(format!("failed to register ioevent: {e:?}")))
                })?;
            } else {
...
impl DeviceManager {
    ...
    fn add_virtio_pci_device(
        ...
        let msix_num = {
            ...
            virtio_device.queue_max_sizes().len() + usize::from(virtio_device.doorbells_max())
        };

        if msix_num >= usize::from(u16::MAX - 1) {
            return Err(DeviceManagerError::TooManyInterrupts(msix_num));
        }
        let msix_num = msix_num as u16 + 1;
        ...
        virtio_pci_device
            .lock()
            .unwrap()
            .ioeventfds(bar_addr, &mut |event, addr| {
                let io_addr = IoEventAddress::Mmio(addr);
                self.address_manager
                    .vm
                    .register_ioevent(event, &io_addr, None)
    ...
    pub fn eject_device(&mut self, pci_segment_id: u16, device_id: u8) -> DeviceManagerResult<()> {
        ...
        let (pci_device, bus_device, virtio_device, remove_dma_handler) = match pci_device_handle {
            ...
            PciDeviceHandle::Virtio(virtio_pci_device) => {
                ...
                dev.ioeventfds(bar_addr, &mut |event, addr| {
                    let io_addr = IoEventAddress::Mmio(addr);
                    self.address_manager
                        .vm
