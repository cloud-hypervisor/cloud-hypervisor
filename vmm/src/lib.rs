            ...
            match req.command() {
                ...
                Command::Config => {
                    ...
                    if let Some(ref restored_net_configs) = receive_data_migration.net_fds {
                        ...
                        {
                            debug!("restored_net_configs={:?}", restored_net_configs.iter().map(|n| n.id.clone()).collect::<Vec<_>>());
                            for net in restored_net_configs {
                                debug!("vm_config.net={:?}", vm_config.net.iter().flatten().map(|n| n.id.clone()).collect::<Vec<_>>());
