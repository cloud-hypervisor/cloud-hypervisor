...
impl Vmm {
    ...
    // Try to find a timing of sending dirty log which actually sends downtime smaller than
    // downtime_limit in the migration_timeout window.
    fn memory_copy_iterations(
        ...
        loop {
            ...
            // Update bandwidth
            if !transfer_time.is_zero() && s.pending_size > 0 {
                bandwidth = s.pending_size as f64 / transfer_time.as_millis() as f64;
