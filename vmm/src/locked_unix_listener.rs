...
/// kernel when the process exits, including on a crash. Dropping the guard
/// unlinks the socket file; the lock is still held while it does so, so it
/// cannot remove a socket bound by another instance.
...
impl LockedUnixListener {
    ...
    pub(crate) fn bind(socket_path: &Path) -> Result<Self, LockedUnixListenerError> {
        ...
        // We hold the lock, so a socket here is stale from a crash; remove
        // it. Only unlink an actual socket, not another file at this path.
        if fs::symlink_metadata(socket_path).is_ok_and(|m| m.file_type().is_socket()) {
...
mod unit_tests {
    ...
    fn test_socket_in_use_is_protected() {
        ...
        // Dropping the guard releases the lock, so the path can be taken again.
        drop(socket);
        LockedUnixListener::bind(&path).unwrap();
    }
    ...
    fn test_drop_unlinks_the_socket() {
        ...
        // The guard unlinks its socket file on drop.
        assert!(!path.exists());
    }
