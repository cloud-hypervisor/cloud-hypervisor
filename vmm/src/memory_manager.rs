...
/// THP policy. Ranges must have passed [`is_restore_cow_compatible`].
...
mod tests {
    ...
    fn cow_restore_maps_data_and_reads_holes_as_zero() {
        ...
        do_mmap_cow_saved_regions(&gm, &file, &table, false, false).unwrap();
    ...
    #[test]
    fn cow_restore_honors_reserve_and_thp() {
        let page = page_size();
        let gm = GuestMemoryMmap::from_ranges(&[(GuestAddress(0), page as usize)]).unwrap();
        let mut file = tempfile::tempfile().unwrap();
        ...
        do_mmap_cow_saved_regions(&gm, &file, &table, true, true).unwrap();
