...
impl SendAdditionalConnections {
    ...
    pub(crate) fn send_memory(
        ...
        if self.threads.is_empty() {
            // Using chunks ensures more frequent "ok" checks and we can exit
            // the transmission earlier if needed.
            for chunk in table.partition(Self::CHUNK_SIZE) {
                send_memory_ranges(&self.guest_memory, &chunk, socket, pending_action.as_ref())?;
            }
            return Ok(true);
        }
...
pub(crate) fn send_memory_ranges(
    ...
    // "happy path abort" before we write the memory table header
    if pending_action.lock().unwrap().is_some() {
        debug!("Stopping to send memory: there is a pending action");
        return Ok(());
    }
