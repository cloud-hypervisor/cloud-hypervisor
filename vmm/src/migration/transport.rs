...
use crate::migration::worker::CANCEL_REQUESTED;
...
impl SendAdditionalConnections {
        ...
        loop {
            ...
            match message {
                SendMemoryThreadMessage::Memory(table) => {
                    ...
                    send_memory_ranges(guest_memory, &table, socket, cancel_state)
                        .inspect_err(|_| {
                            worker_error.store(true, Ordering::Release);
                            notify_tx.send(SendMemoryThreadNotify::Error).ok();
                        })
    ...
    pub(crate) fn send_memory(
        ...
        if self.threads.is_empty() {
            send_memory_ranges(&self.guest_memory, &table, socket, cancel_state)?;
            return Ok(true);
        }
...
pub(crate) fn send_memory_ranges(
    ...
    for range in ranges.regions() {
        ...
        loop {
            ...
            if cancel_state.load(Ordering::Acquire) == CANCEL_REQUESTED {
