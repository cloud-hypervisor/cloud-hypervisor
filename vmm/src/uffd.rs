...
pub(crate) fn create(required_features: u64) -> Result<RawFd, std::io::Error> {
    ...
    let ret = unsafe { libc::ioctl(fd, userfaultfd::UFFDIO_API as libc::Ioctl, &mut api) };
    if ret < 0 {
...
pub(crate) fn register(fd: RawFd, addr: u64, len: u64) -> Result<u64, std::io::Error> {
    ...
    let ret =
        unsafe { libc::ioctl(fd, userfaultfd::UFFDIO_REGISTER as libc::Ioctl, &mut reg) };
    if ret < 0 {
        return Err(std::io::Error::last_os_error());
    }
    Ok(reg.ioctls)
...
pub(crate) fn copy(fd: RawFd, dst: u64, src: *const u8, len: u64) -> Result<(), std::io::Error> {
    ...
    let ret = unsafe { libc::ioctl(fd, userfaultfd::UFFDIO_COPY as libc::Ioctl, &mut cp) };
    if ret < 0 {
        return Err(std::io::Error::last_os_error());
    }
    Ok(())
...
pub(crate) fn wake(fd: RawFd, addr: u64, len: u64) -> Result<(), std::io::Error> {
    let mut range = UffdioRange { start: addr, len };
    // SAFETY: `range` is a valid, correctly-sized struct for this ioctl.
    let ret = unsafe { libc::ioctl(fd, userfaultfd::UFFDIO_WAKE as libc::Ioctl, &mut range) };
    if ret < 0 {
        return Err(std::io::Error::last_os_error());
    }
    Ok(())
